WITH stats AS (
    SELECT *
    FROM {{  ref('fct_player_stats_per_75')  }}
)
, eligible AS (
    SELECT s.*
    FROM stats s
...
, ranked AS (
    SELECT player_id
        ...
        , league_id
        , season_type
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY pts_per_75) AS pts_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY fgm_per_75) AS fgm_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY fga_per_75) AS fga_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY fg_pct) AS fg_pct_percentile

        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY fg3m_per_75) AS fg3m_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY fg3a_per_75) AS fg3a_per_75_percentile
        ...
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY fg2m_per_75) AS fg2m_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY fg2a_per_75) AS fg2a_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY fg2_pct) AS fg2_pct_percentile

        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY ftm_per_75) AS ftm_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY fta_per_75) AS fta_per_75_percentile
        ...
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY oreb_per_75) AS oreb_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY dreb_per_75) AS dreb_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY reb_per_75) AS reb_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY ast_per_75) AS ast_per_75_percentile

        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY stl_per_75) AS stl_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY blk_per_75) AS blk_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY stk_per_75) AS stk_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY tov_per_75) AS tov_per_75_percentile
        , PERCENT_RANK() OVER (PARTITION BY season, league_id, season_type ORDER BY pf_per_75) AS pf_per_75_percentile
...
SELECT s.player_id, s.team_id, s.season, s.league_id, s.season_type, s.games_played
    , r.pts_per_75_percentile
    , r.fgm_per_75_percentile
    , r.fga_per_75_percentile
    , r.fg_pct_percentile

    , r.fg3m_per_75_percentile
    , r.fg3a_per_75_percentile
    , r.fg3_pct_percentile

    , r.fg3pr_percentile

    , r.fg2m_per_75_percentile
    , r.fg2a_per_75_percentile
    , r.fg2_pct_percentile

    , r.ftm_per_75_percentile
    , r.fta_per_75_percentile
    , r.ft_pct_percentile
    , r.ftr_percentile

    , r.oreb_per_75_percentile
    , r.dreb_per_75_percentile
    , r.reb_per_75_percentile
    , r.ast_per_75_percentile

    , r.stl_per_75_percentile
    , r.blk_per_75_percentile
    , r.stk_per_75_percentile
    , r.tov_per_75_percentile
    , r.pf_per_75_percentile
